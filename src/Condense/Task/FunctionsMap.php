...
class FunctionsMap extends TaskAbstract
{
		...
		)
	{
		...
		$packages->setStorage($storage); // only for ProtoPackages

		$allPackages = $packages->getPackages();
		foreach ($allPackages as $package)
		{
			$def = $packages->definition( $package );
			$this->mapPackage($def, $composer, $storage);
	...
	protected function mapPackage(array $definition, $composer, $storage)
	{
		foreach($definition as $lib => $classes)
		{
